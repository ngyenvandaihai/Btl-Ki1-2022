...
class Sanpham extends CI_Controller {
    ...
    public function producer(){
        $aurl= explode('/',uri_string());
        $link=$aurl[1];
        $producer=$this->Mproducer->producer_id($link);
        // $listcat=$this->Mproducer->producer_listcat($catid);
        $this->data['producername']=$this->Mproducer->producer_name($producer);
        if(isset($_POST['sapxep-producer'])){
            $dksx=$_POST['sapxep-producer'];
            $char = explode('-', $dksx);
            $f=$char[0];
            $od=$char[1];
            $data = array('0' => $f,
            ...
            $this->session->set_userdata('sortby-producer', $data);
        }else{
            if($this->session->userdata('sortby-producer')){
                $data = $this->session->userdata('sortby-producer');
                $f=$data[0];
                $od=$data[1];
                $array = array('sortby');
            }else{
